...
class MLAnomalyDetector:
    ...
    def detect(self, session_id: str) -> int:
        ...
        if len(transactions) < 50:
            print(
                f"⚠️ Only {len(transactions)} transactions, need 50+ for ML detection")
            return 0
        ...
        for idx, (pred, score) in enumerate(zip(predictions, anomaly_scores)):
            if pred == -1:  # Anomaly detected
                ...
                if abs(txn.amount) > MAX_AMOUNT_FOR_ANALYSIS:
                    print(
                        f"⚠️ Skipping anomaly for extreme amount: ${abs(txn.amount):.2f}")
                    continue
    ...
    def _extract_features(self, transactions: list) -> pd.DataFrame:
        ...
        df['amount_zscore'] = (df['amount'] - amount_mean) / \
            (amount_std if amount_std > 0 else 1)
    ...
    def _generate_ml_explanation(
        ...
        # Check amount - high or low
        is_high = features['amount_zscore'] > 2.0
        is_low = features['amount_zscore'] < -1.5

        # Check if one-time or rare merchant
        is_rare_merchant = features['is_one_time'] == 1 or features['merchant_frequency'] < 0.02

        # Build user-friendly explanation based on transaction type
        if is_income:
            # Income transaction explanations
            if is_high:
                explanation = (
                    f"This ${amount:.0f} {category_name} deposit is higher than your usual income. "
                    f"Worth checking if it's a bonus, refund, or expected payment."
                )
            elif is_low:
                explanation = (
                    f"This ${amount:.0f} {category_name} deposit is lower than usual. "
                    f"You might want to verify it's the correct amount."
                )
            else:
                explanation = (
                    f"This ${amount:.0f} {category_name} deposit stands out from your usual income pattern."
                )
        else:
            # Expense transaction explanations
            if is_high and is_rare_merchant:
                explanation = (
                    f"This ${amount:.0f} {category_name} purchase is larger than usual "
                    f"and from somewhere you don't shop often."
                )
            elif is_high:
                explanation = (
                    f"This ${amount:.0f} {category_name} purchase is noticeably higher "
                    f"than what you typically spend here."
                )
            elif is_low:
                explanation = (
                    f"This ${amount:.0f} {category_name} charge is unusually low "
                    f"compared to your typical spending."
                )
            elif is_rare_merchant:
                explanation = (
                    f"This ${amount:.0f} {category_name} purchase is from "
                    f"a merchant you don't use often."
                )
            else:
                explanation = (
                    f"This ${amount:.0f} {category_name} charge looks different "
                    f"from your usual spending pattern."
                )
...
class AnomalyDetector:
    ...
    def detect(self, session_id: str) -> int:
        ...
        if len(transactions) >= 50 and self.ml_detector is not None:
            print(
                f"🤖 Using ML anomaly detection ({len(transactions)} transactions)")
            return self.ml_detector.detect(session_id)

        # Fallback to statistical z-score method
        print(
            f"📊 Using statistical anomaly detection ({len(transactions)} transactions)")
    ...
    def _generate_explanation(
        ...
        Args:
            ...
            z_score_abs: Absolute z-score value (unused but kept for interface).
